...
**Verified — bounded filesystem mapping.** Directory-only PFTP reads succeeded for `/`, `/U/`, and `/U/0/`. The `/U/0/` listing included date-shaped directories plus `AUTOS/`, `DGOAL/`, `NR/`, `SLEEP/`, `SPROF/`, `S/`, `TL/` and user-related metadata entries. Public documentation intentionally does not publish user-specific date names or identifiers.
